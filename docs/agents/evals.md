...
- The headline eval should mirror real failure modes: Java stream code that may materialize
...
- Every headline criterion must classify `category` as `safety`, `stream_quality`, or
  `maintainability`.
- Headline implementation scenarios need compile/artifact and behavior checks as safety checks, but
  the public benchmark should be weighted toward `stream_quality`.
- For headline scenarios, use roughly `15` safety points, `80` stream-quality points, and `5`
...
- Use Sonnet 4.6 for this repository's headline eval runs:
