...
set and later moved to `evals-regression/`. It requires access to the skill-provided scan header and
`rg` command, so report it as with-context workflow regression coverage rather than as main or
reference Java stream reasoning lift.
...
`evals-regression/`. They are context-dependent workflow scenarios that require exact
skill-provided scan text, so they are useful with-context workflow regression checks but should not
be counted as fair without-context reference lift evidence.
